...
بيانات «راتبي» الحالية لا تحتوي قائمة budgets مستقلة، لذلك نفّذ التالي دون اختراع أرقام:

1. أضف داخل قسم «ربط رُشد» حقلين اختياريين:
   - `ميزانية الأماني الشهرية`
   - `المستخدم من ميزانية الأماني هذا الشهر`
2. احفظهما في settings بالمفتاحين:
   - `rushdWishesBudget`
   - `rushdWishesSpent`
3. إذا كانت الميزانية أكبر من صفر، أرسل:

```js
{
  id: 'wishes',
  title: 'أماني رُشد',
  limit: rushdWishesBudget,
  spent: Math.min(rushdWishesSpent, rushdWishesBudget),
  kind: 'wishes'
}
```

4. أنشئ ميزانية مرنة من المتبقي بعد الالتزامات والأهداف:
